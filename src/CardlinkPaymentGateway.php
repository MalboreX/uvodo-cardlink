...
/** @package Plugins\Cardlink */
class CardlinkPaymentGateway implements OtherPaymentGatewayInterface
{
    public const SHORT_NAME = "cardlink";
    private string $host;
    ...
    private function createToken(): mixed
    {
        ...
        $req = $this->httpRequestFactory->createRequest('POST', $uri)
            ...
            ->withHeader(
                'Authorization',
                'Basic ' . base64_encode($this->clientId->getValue() . ':' . $this->apiKey->getValue())
            )->withBody($body);
